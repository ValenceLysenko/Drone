...
class TelloFullPanel(QWidget):
    ...
    def initUI(self):
        ...
        video_tools = QWidget()
        video_tools_lay = QGridLayout(video_tools)
        video_tools_lay.setContentsMargins(0, 8, 0, 0)
        video_tools_lay.setSpacing(5)

        filter_lbl = QLabel("VIDEO FILTER")
        filter_lbl.setStyleSheet("color: #00d4ff; font-weight: bold; font-size: 10px;")
        ...
        self.filter_combo.addItem("Thermal", "thermal")
        self.filter_combo.addItem("Night boost", "night")
        self.filter_combo.currentIndexChanged.connect(self.change_video_filter)
        self.filter_combo.setStyleSheet(
            "background-color: white; color: #111; min-height: 34px; border-radius: 4px; padding-left: 6px;")

        self.btn_snapshot = self.create_btn("Snapshot")
        self.btn_snapshot.clicked.connect(self.capture_snapshot)
        self.btn_snapshot.setStyleSheet("background-color: #0288d1; color: white;")

        self.btn_record = self.create_btn("REC OFF")
        self.btn_record.clicked.connect(self.toggle_recording)
        self.btn_record.setStyleSheet("background-color: #455a64; color: white;")

        self.lbl_fps = QLabel("FPS: --")
        self.lbl_fps.setStyleSheet("color: white; font-weight: bold; background: transparent;")

        video_tools_lay.addWidget(filter_lbl, 0, 0, 1, 3)
        video_tools_lay.addWidget(self.filter_combo, 1, 0, 1, 3)
        video_tools_lay.addWidget(self.btn_snapshot, 2, 0)
        video_tools_lay.addWidget(self.btn_record, 2, 1)
        video_tools_lay.addWidget(self.lbl_fps, 2, 2, alignment=Qt.AlignmentFlag.AlignCenter)
        u_lay.addWidget(video_tools, 3, 0, 1, 3)
    ...
    def change_video_filter(self, index=None):
        mode = self.filter_combo.currentData()
        self.video_thread.set_filter_mode(mode)
        self.terminal_display.setText(f" > VIDEO FILTER: {self.filter_combo.currentText().upper()}")

    def capture_snapshot(self):
        path = self.video_thread.save_snapshot()
        if path:
            self.terminal_display.setText(f" > SNAPSHOT SAVED: {path}")
        else:
            self.terminal_display.setText(" > SNAPSHOT FAILED: NO VIDEO FRAME")

    def toggle_recording(self):
        if self.is_recording:
            self.video_thread.stop_recording()
        else:
            path = self.video_thread.start_recording()
            self.terminal_display.setText(f" > RECORDING REQUESTED: {path}")

    def handle_recording_state(self, active, message):
        self.is_recording = active
        if active:
            self.btn_record.setText("REC ON")
            self.btn_record.setStyleSheet("background-color: #c62828; color: white;")
            self.terminal_display.setText(f" > RECORDING STARTED: {message}")
        else:
            self.btn_record.setText("REC OFF")
            self.btn_record.setStyleSheet("background-color: #455a64; color: white;")
            self.terminal_display.setText(f" > RECORDING STOPPED: {message}")

    def update_video_stats(self, stats):
        if "fps" in stats:
            self.lbl_fps.setText(f"FPS: {stats['fps']}")
